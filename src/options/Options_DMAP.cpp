...
#include "Options_DMAP.h"

namespace options {

bool Options_DMAP::process(int argc, char *argv[]) {
	...
	// PROCESS PARAMETERS
	stringstream ss;
	ss << package_description() << endl << endl << "This is ERNE-DMAP. Allowed options";
...
//		("errors-delta", po::value<t_errors_delta>(), "errors delta (>= 0, default 0)")
		...
		("min-phred-value-CLC",po::value<Mask::t_min_phred_value_MOTT>(),"minimum mean value used by CLC-like trimming")
		("min-mean-phread-quality",po::value<Mask::t_min_phred_value_MOTT>(),"minimum mean value to accept a sequence")
		("min-size", po::value<t_pattern_length>(), "min length for a sequence (default 25)")
		("gap",po::value<t_length>(),"Efficiently search 1 gap inside an interval of arg bases (NOT IMPLEMENTED YET)")
		("transcriptome","detect correct intron-exons junctions in transcriptome-seq (require --gap) (NOT IMPLEMENTED YET)")
